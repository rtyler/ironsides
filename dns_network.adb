...
--# hide DNS_Network;
   ...
   begin
      ...
      Gnat.Sockets.Set_Socket_Option(UDP_Socket,
...
--         Gnat.Sockets.Get_Host_By_Name(Gnat.Sockets.Host_Name), 1);
   ...
   is
      function Convert is new Ada.Unchecked_Conversion(DNS_Socket,
         Socket_Timeout.Socket_Type);
   begin
   ...
   is
      ...
      Item    : Ada.Streams.Stream_Element_Array (1 .. DNS_Types.DNS_Packet'Size/8);
   ...
   begin
      ...
      Failure := (Number_Bytes < DNS_Types.Packet_Length_Range(1+DNS_Types.Header_Bits/8))
         OR (Number_Bytes > MAX_QUERY_SIZE);
   ...
   is
      USE TYPE Dns_Types.Packet_Length_Range;
      use type ada.streams.Stream_Element_Offset;
      Response_Stream : Ada.Streams.Stream_Element_Array (1 .. DNS_Types.DNS_Packet'Size/8);
   ...
   begin
      ...
      Gnat.Sockets.Send_Socket(
         Socket => Gnat.Sockets.Socket_Type(Socket),
         Item   => Response_Stream (1 .. Ada.Streams.Stream_Element_Offset(Number_Bytes+2)),
         Last   => Result_Last);
--      put_line("Num bytes sent" & Ada.Streams.Stream_Element_Offset'image(result_last));
      if Result_Last /= Ada.Streams.Stream_Element_Offset(Number_Bytes+2) then
         Failure := True;
      else
         Failure := False;
      end if;
...
--      put_line("Num bytes" & dns_types.packet_length_range'image(number_bytes));
      if System.Default_Bit_Order=System.Low_Order_First then
...
--         Server => Gnat.Sockets.Sock_Addr_Type(To_Address));
      ...
      Gnat.Sockets.Send_Socket(
         Socket => UDP_Socket,
         Item   => Response_Stream (1 .. Ada.Streams.Stream_Element_Offset(Number_Bytes)),
         Last   => Result_Last,
         To     => Gnat.Sockets.Sock_Addr_Type(To_Address));
--      put_line("Num bytes" & Ada.Streams.Stream_Element_Offset'image(result_last));
